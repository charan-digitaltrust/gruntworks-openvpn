...
func TestOpenVpnInitializationSuite(t *testing.T) {
	...
	testSuite.resourceCollection = resources.CreateBaseRandomResourceCollection(t, "ap-northeast-1")
	...
	testSuite.logger.Println("SetupSuite Complete, Running Tests")
...
func testOpenVpnAdminProcessRequestsIsRunning(t *testing.T, testSuite *suite) {
	...
	testSuite.output, err = ssh.CheckSshCommand(testSuite.host, commandToTest, testSuite.logger)
	if err != nil {
		t.Fatalf("Failed to SSH to EC2 Instance at %s and execute command :%s\n", testSuite.ipAddress, err.Error())
	}
